...
export function nearestFree(map,buildings,x,y,avoidMountains=false,avoidForests=avoidMountains){
  const W=map.width??96,H=map.height??64;
  ...
  for(let r=0;r<12;r++) for(let dy=-r;dy<=r;dy++) for(let dx=-r;dx<=r;dx++){
    ...
    if(walkable(map,buildings,x+dx,y+dy,avoidMountains,avoidForests))return {x:x+dx+.5,y:y+dy+.5};
  }
  return null;
}
...
export function findPath(map,buildings,start,end,avoidMountains=false,avoidForests=avoidMountains){
  const W=map.width??96,H=map.height??64;
  const dest=nearestFree(map,buildings,end.x,end.y,avoidMountains,avoidForests);if(!dest)return [];
  ...
  while(heap.a.length){
    ...
    for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){
      ...
      if(!walkable(map,buildings,nx,ny,avoidMountains,avoidForests))continue;
      if(dx&&dy&&(!walkable(map,buildings,x+dx,y,avoidMountains,avoidForests)||!walkable(map,buildings,x,y+dy,avoidMountains,avoidForests)))continue;
