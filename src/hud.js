...
function update({state,selected,view,paused,speed,level,interaction}){
  ...
  $('result').hidden=!state.result||sandbox;if(state.result){
    const copy={sandbox:['红方全灭','蓝方获胜，可回到编辑调整布阵。','蓝方全灭，可回到编辑调整布阵。'],balanced:['敌建筑全毁','敌方全部建筑已摧毁，均衡对抗胜利。','保护经济建筑，集结部队后再出击。'],demo:['敌营已摧毁','两座敌营已摧毁，本次行动胜利。','调整阵型，保护弓箭兵，再试一次。'],attack:['敌建筑全毁','敌方建筑全部摧毁，进攻胜利。','敌军防守严密，尝试先削弱其经济或集火逐个击破。'],defend:['防线守住了','两波敌军已全部消灭，我方仍有建筑存活。','防线被突破，试试哨塔与弓箭兵配合。']}[state.level==='randomAttack'?'attack':state.level||level];
