...
function update({state,selected,view,paused,speed,level,interaction}){
  ...
  for(const [id,s] of Object.entries(TECHNOLOGIES)){
    const tech=state.technologies[id],button=$('research-'+id),cost=s.ore?`${s.food} 食物 / ${s.ore} 矿`:`${s.food} 食物`,status=tech.status==='complete'?'已完成':tech.status==='researching'?`研发中 · ${Math.ceil(tech.remaining)} 秒`:`${cost} · ${s.researchTime} 秒`;
    const unlock={castIron:'解锁装甲车',artillery:'蒸汽步行机前置',steamCore:'蒸汽步行机前置',compositeShield:'盾兵升级为铁甲兵',precisionBolts:'弓箭兵升级为强弩兵',birdTraining:'解锁信鸽'}[id];
  ...
  $('building-info').textContent=techMenu?'各分区科技均可并行研发；开始即扣除资源。':building?`生命 ${Math.ceil(building.hp)} / ${building.maxHp} · 护甲 ${STATS[building.type].armor} · ${building.awaitingEviction?'等待区域内部队离开，随后自动施工':building.constructionPending?(building.activeBuilders?`施工 ${building.activeBuilders} 人 · 预计剩余 ${Math.ceil(building.constructionRemaining/building.activeBuilders)} 秒`:'等待施工人员到场 · 可选中部队右键补派'):building.type==='base'?`不产资源；${STATS.base.healRange} 格内最多治疗 ${STATS.base.healTargets} 人，每人每秒 +${STATS.base.healRate} 生命。${['attack','randomAttack','defend','tutorial'].includes(state.level)?`本关固定 ${state.popCap} 人口。`:`提供 ${STATS.base.pop} 人口。`}按 Y 设置集结点${building.rallyPoint?` · 当前 ${building.rallyPoint.x.toFixed(1)}, ${building.rallyPoint.y.toFixed(1)}`:''}。`:building.type==='machineFactory'?`生产机械单位。按 Y 设置集结点${building.rallyPoint?` · 当前 ${building.rallyPoint.x.toFixed(1)}, ${building.rallyPoint.y.toFixed(1)}`:''}。`:building.type==='mine'?`每秒 +${STATS.mine.oreRate} 矿产`:building.type==='factory'?`每秒 +${foodRate(state.map,building)===6?'6 食物（食物点 ×2）':'3 食物'}`:`入驻 ${1+state.units.filter(u=>u.hp>0&&u.garrisonId===building.id).length}/4 人 · E 全部退出 · 自带 1 名${STATS[towerGarrisonType(state,building.team)].name} · 视野 ${STATS.tower.vision} / 射程 ${STATS.tower.range}`}`:buildType?`左键放置${STATS[buildType].name}，绿色可建 / 红色不可建。`:'C 基地 / R 采矿场 / Q 哨塔 / F 食物厂 / M 机械工厂。';
  ...
  $('result').hidden=!state.result||sandbox;if(state.result){
    const copy={sandbox:['红方全灭','蓝方获胜，可回到编辑调整布阵。','蓝方全灭，可回到编辑调整布阵。'],balanced:['敌建筑全毁','敌方全部建筑已摧毁，均衡对抗胜利。','保护经济建筑，集结部队后再出击。'],tutorial:['教程完成','你已掌握核心操作，进阶技巧可随时查看「操作速查」。','教程中断：基地已失守，重新开始再练习一次。'],attack:['敌建筑全毁','敌方建筑全部摧毁，进攻胜利。','敌军防守严密，尝试先削弱其经济或集火逐个击破。'],defend:['防线守住了','两波敌军已全部消灭，我方仍有建筑存活。','防线被突破，试试哨塔与弓箭兵配合。']}[state.level==='randomAttack'?'attack':state.level||level];
